...
var Token = TokenService(PaperWalletClient());
async function getPaperToken(req, res) {
  const { code } = req.body;
  try {
    const userToken = await Token.getToken(code);
    return res.status(import_http_status.OK).json({ userToken });
  } catch (e) {
    console.error({ e });
    return res.status(import_http_status.INTERNAL_SERVER_ERROR).json(e);
  }
}
