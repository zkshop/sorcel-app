...
__export(src_exports, {
  getPaperWallet: () => getPaperWallet
});
module.exports = __toCommonJS(src_exports);

// ../../../../../packages/domains/auth/AuthService.ts
...
var initialAuthData = { email: null, issuer: null, phoneNumber: null, publicAddress: null };
...
var Auth = AuthService(UserAuthenticationClient());
async function getPaperWallet(req, res) {
  const { code } = req.body;
  try {
    const paperWallet = await Auth.loginWithPaper(code);
    return res.status(import_http_status.OK).json(paperWallet);
  } catch (e) {
    console.error(e);
    return res.status(import_http_status.INTERNAL_SERVER_ERROR).send({ message: "Error when requesting the wallet " });
  }
}
