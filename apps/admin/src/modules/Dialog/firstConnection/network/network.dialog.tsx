...
import { dialog, useDialog } from "@3shop/ui/Modal/Dialogs";
import { Box, Button, Flex, Spinner, Text } from "@3shop/ui";
...
const Content = () => {
  ...
  const handleNetworkSelection = (networkValue: Network_standard) => {
    ...
    attachSubmit(() => {
      ...
      if (currentSettings)
        set({ ...currentSettings, network: networkValue });
      else
        set({ network: networkValue });
    });
    setNetwork(networkValue);
  };

  // if (loading)
    // return <Flex justify="center" align="center"><Spinner /></Flex>;
  return (
    <>
      ...
      <Flex direction="column" align="center" width={"100%"}>
        <Text textAlign="center" opacity={"0.75"}>You can change this preference at any time in the integration tab.</Text>
      </Flex>
    </>
  );
};
