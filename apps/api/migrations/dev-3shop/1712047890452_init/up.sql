...
CREATE TABLE public.chain_type (
    value text NOT NULL
);
...
CREATE TABLE public.wallet_connection_log (
    id integer NOT NULL,
    app_id uuid NOT NULL,
    address text NOT NULL,
    date timestamp with time zone DEFAULT now() NOT NULL
);
COMMENT ON TABLE public.wallet_connection_log IS 'log date and time uppon wallet connection. stores the address';
CREATE SEQUENCE public.wallet_connection_logs_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER SEQUENCE public.wallet_connection_logs_id_seq OWNED BY public.wallet_connection_log.id;
ALTER TABLE ONLY public.wallet_connection_log ALTER COLUMN id SET DEFAULT nextval('public.wallet_connection_logs_id_seq'::regclass);
ALTER TABLE ONLY public.gate
    ADD CONSTRAINT "Gate_pkey" PRIMARY KEY (id);
ALTER TABLE ONLY public.app
    ADD CONSTRAINT app_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.chain_type
    ADD CONSTRAINT chain_type_pkey PRIMARY KEY (value);
...
ALTER TABLE ONLY public.wallet_connection_log
    ADD CONSTRAINT wallet_connection_logs_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.app
...
ALTER TABLE ONLY public.wallet_connection_log
    ADD CONSTRAINT wallet_connection_logs_app_id_fkey FOREIGN KEY (app_id) REFERENCES public.app(id) ON UPDATE RESTRICT ON DELETE RESTRICT;
